...
def addops(tac, TAC, symTab, M_Code):
	if tac[3] == 'int:=' or tac[3] == 'integer:=':
		...
		if type(tac[2]) is float:
			M_Code.create_new_line(['li.s', '$f30', tac[2]])  # don't use $f30 anywhere
			M_Code.create_new_line(['add.s', r1, r2, '$f30'])  # don't use $f30 anywhere
		else:
		...
		if type(tac[2]) is float:
			M_Code.create_new_line(['li.s', '$f30', tac[2]])  # don't use $f30 anywhere
			M_Code.create_new_line(['sub.s', r1, r2, '$f30'])  # don't use $f30 anywhere
		else:
			r3 = M_Code.return_function_register(tac[2], 3)
...
def output(tac, TAC, symTab, M_Code):
	if tac[3] == 'WRITE_NL' :
		...
		M_Code.create_new_line(['li', '$v0', '5', ''])
		M_Code.create_new_line(['syscall', '', '', ''])
		r1 = M_Code.return_register(tac[1],1)
		r2 = '$v0'
		M_Code.create_new_line(['move',r1,r2,''])
		...
		M_Code.create_new_line(['li', '$v0', '6', ''])
		M_Code.create_new_line(['syscall', '', '', ''])
		r1 = M_Code.return_function_register(tac[1],1)
		r2 = '$f0'
		M_Code.create_new_line(['mov.s',r1,r2,''])
		M_Code.clear_func_register(r1)
...
def mips_instr_generator(TAC,symTab): #code_gen
