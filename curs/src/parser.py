...
def p_type_denoter_1(p):
	'type_denoter :  identifier'	# integer, real, string
	p[0] = {}
...
def p_variable_declaration_1(p):
	...
	for iden in p[1]['list_id']:
		...
		if st_entry is not None:
			...
			S_TABLE.currentScope.re_get_identificator(name=iden,id_dict={'type':p[3]['type'],'t_name':S_TABLE.create_temporarly(name=iden,typ=p[3]['type'],width=p[3]['width'],s_entry=st_entry)})
			p[0]['type'] = 'VOID'
	...
	if st_entry['type'] != 'typedef':
		...
		for iden in p[1]['list_id']:
			...
			if st_entry is not None:
				...
				st_entry = S_TABLE.currentScope.get_identificator(name=iden)
				nt = S_TABLE.create_temporarly(name=iden,typ=p[3]['name'],width=type_width,s_entry=st_entry)
...
def p_simple_expression_2(p):
	...
	# for other operators we are going to type-cast it into the largest-sized data-type .
		...
		TAC.create_tac_record(p[0]['t_name'], temp_real(p[1]), temp_real(p[3]), 'real'+p[2]['name'])	# temp_real function is used for handling type-conversion during assignment
	elif p[1]['type'] == 'integer' or p[3]['type'] == 'integer' :
		p[0]['type'] = 'integer'					# integer is a larger data-type
		TAC.create_tac_record(p[0]['t_name'], temp_integer(p[1]), temp_integer(p[3]), 'int'+p[2]['name'])	# temp_integer function is used for handling type-conversion during assignment
...
def p_function_heading_1(p):
	...
	if st_entry is not None:
		...
		S_TABLE.currentScope.re_get_identificator(name=p[2]['name'],id_dict={'type':'function','result_type':p[4]['type'],'label':p[0]['label'],'type_list':[],'arg_type_list':[],'arg_temp_list':[],'t_name':S_TABLE.create_temporarly(name=p[2]['name'],width=0,s_entry=st_entry),'param_width':0})
		p[0]['type'] = 'VOID'
...
def p_function_heading_2(p):
	...
	p[3]['f_st_entry']['t_name'] = S_TABLE.create_temporarly(name=p[2]['name'],typ='function',s_entry=p[3]['f_st_entry'],width=4)
...
def p_function_identification_1(p):
	...
	if st_entry is not None:
		...
		st_entry = S_TABLE.currentScope.get_identificator(name=p[2]['name'])
		S_TABLE.currentScope.re_get_identificator(name=p[2]['name'],id_dict={'type':'function','result_type': 'VOID','type_list':[],'t_name':S_TABLE.create_temporarly(name=p[2]['name'],typ='function',s_entry=st_entry,width=0),'param_width':0})
		p[0]['type'] = 'VOID'
		S_TABLE.start_view_range(name=p[2]['name'])
...
def p_procedure_statement_1(p):
	...
	if(st_entry == None) :
		if p[1]['name'].lower() == 'write':
			...
			for param_type, param in zip(p[2]['type_list'], p[2]['list']):
				if(param_type == 'integer'):
					TAC.create_tac_record('', param['t_name'],'','READ_INT')
				elif(param_type == 'real'):
					TAC.create_tac_record('', param['t_name'],'','READ_REAL')
				elif(param_type == 'string'):
					TAC.create_tac_record('', param['t_name'],'','READ_STRING')
				else :
...
def p_unsigned_constant_2(p):
	...
	p[0]['t_name'] = S_TABLE.create_temporarly()
	TAC.create_tac_record(p[0]['t_name'],p[1],'','string:=')	# issue: how to get the exact string value ??
...
def p_factor_1(p):
	...
	if p[2]['type'] == 'string' :
		...
		p[0]['type'] = p[2]['type']
		TAC.create_tac_record(p[0]['t_name'],0,p[2]['t_name'],p[1]['name'])		# implementing unary operators by substracting it from 0
...
def p_function_designator_1(p):
	...
	if(st_entry == None) :
		...
		rt = S_TABLE.create_temporarly(name = p[1]['name'],width=4,typ='function',s_entry=st_entry)
...
class ThreeAddrCode:
	...
	def create_tac_record(self,dest,src1,src2,op):  #emit
	...
	def create_func_record(self,func_name):  #add_func
		self.code[func_name] = []
		self.quad[func_name] = -1
		self.n_quad[func_name] = 0

	def print_three_adr_code(self):   #print_TAC
		for t in self.code:
			print(t)
