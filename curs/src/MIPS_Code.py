class MIPS_Code(object):
	...
	def return_register(self,temp,arg_num):  #get_reg
		reg = '$t'+str(arg_num)
		...
		if curr_temp_details['array_access']:
			self.create_new_line(['lw','$t9', str(curr_temp_details['offset'])+'($sp)' ,''])
			self.create_new_line(['lw',reg,'0($t9)',''])
		else:
			self.create_new_line(['lw',reg, str(curr_temp_details['offset'])+'($sp)' ,''])
		return reg

	def return_register_address(self,temp,arg_num):  #get_addr_reg
		reg = '$t'+str(arg_num)
		self.register_descriptor[reg] = temp
		curr_temp_details = self.symTab.scope_list[self.currFunc].tempList[temp]
	...
	def create_new_data_record(self,temp,data): #add_to_data
		self.data += [temp + data]

	def create_new_line(self,line): #add_line
		self.code[self.currFunc] += [line]

	def return_register_for_function_temporarly(self,temp,arg_num,func_name): #get_reg_for_func_temp
		reg = '$t'+str(arg_num)
		self.register_descriptor[reg] = temp
	...
	def return_register_argument(self,temp,arg_num):  #get_arg_reg
		reg = '$t'+str(arg_num)
		self.register_descriptor[reg] = temp
		curr_temp_details = self.symTab.scope_list[self.currFunc].tempList[temp]
		curr_temp_details['reg'] = reg
	...
	def return_value_address(self,func_name,ret_temp): #set_return_val_a-ddr
	...
	def assing_temporarly_into_register(self, temp, reg) :  #load_temp_in_reg
		self.register_descriptor[reg] = temp
		curr_temp_details = self.symTab.scope_list[self.currFunc].tempList[temp]
		curr_temp_details['reg'] = reg
		self.create_new_line(['lw',reg, str(curr_temp_details['offset'])+'($sp)' ,''])

	def create_new_function(self,f_n): #new_f-unc
		self.code[f_n] = []
		self.currFunc = f_n

	def return_function_register(self,temp,arg_num):  #get_f_reg
		reg = '$f'+str(arg_num)
		self.register_descriptor[reg] = temp
		curr_temp_details = self.symTab.scope_list[self.currFunc].tempList[temp]
		curr_temp_details['reg'] = reg
	...
	def assing_temporarly_into_func_register(self, temp, reg) : #load_temp_in_f_reg
		self.register_descriptor[reg] = temp
		curr_temp_details = self.symTab.scope_list[self.currFunc].tempList[temp]
		curr_temp_details['reg'] = reg
		self.create_new_line(['l.s',reg, str(curr_temp_details['offset'])+'($sp)' ,''])

	def get_assign(self):  #allocate_activation
		curr_func_details = self.symTab.scope_list[self.currFunc]
		self.create_new_line(['move','$fp','$sp',''])

	def clear_func_register(self,reg):   #flush_f_reg
	...
	def clear_register_func_name(self,reg,func_name):  #flush_reg_f-unc
		if self.register_descriptor[reg] is None:
			return
	...
	def return_function_handler(self,func_name): #function_return_handler
		curr_func_details = self.symTab.scope_list[func_name]
	...
	def function_handler_caller(self,callee_name,counter,tac_params,tac_param_type,width,label,ret_temp): #function_caller_handler
	...
	def clear_register(self,reg):  #flush_reg
		if self.register_descriptor[reg] is None:
			return
		curr_temp_details = self.symTab.scope_list[self.currFunc].tempList[self.register_descriptor[reg]]
		self.create_new_line(['sw',reg,  str(curr_temp_details['offset'])+'($sp)' ,''])
	...
	def comms(self, str) : #annotate_code
		annotate = True
		if annotate :
			self.create_new_line(['#', str, '', ''])

	def write_mips_code(self):  #p-rint_code
		print_str =  '.text'+'\n'
